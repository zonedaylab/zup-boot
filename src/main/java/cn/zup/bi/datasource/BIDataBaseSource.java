...
public class BIDataBaseSource implements BaseRbacDataSource {
	...
	@Override
	public SqlSessionFactory baseSqlSessionFactory(@Qualifier("biDataSource") DataSource baseDataSource) {
...
//			log.error("数据库工厂创建失败："+e.getMessage());
	...
	@Override
	public DataSourceTransactionManager baseDataSourceTransactionManager(@Qualifier("biDataSource") DataSource baseDataSource) {
		return new DataSourceTransactionManager(baseDataSource);
	}

	@Bean("biSqlSessionTemplate")
	@Override
	public SqlSessionTemplate baseSqlSessionTemplate(@Qualifier("biSqlSessionFactory") SqlSessionFactory baseSqlSessionFactory) {
