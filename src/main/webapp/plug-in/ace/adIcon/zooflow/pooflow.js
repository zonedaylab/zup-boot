...
function GooFlow(bgDiv,property){
	...
	if(property.haveHead){
		tmp="<div class='GooFlow_head' ><label style='height: 35px;' title='"+(property.initLabelText||"newFlow_1")+"'>"+(property.initLabelText||"newFlow_1")+"</label>";
		for(var x=0;x<property.headBtns.length;++x){
			tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn'><b class='fa fa-save'></b></a>"  //<b class='ico_"+property.headBtns[x]+"'></b>
		}
		tmp+="</div>";
		this.$head=$(tmp);
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "fa fa-save":	if(This.onBtnSaveClick!=null)	This.onBtnSaveClick();break;
	...
	this.$workArea=$("<div  style='width:"+width*2+"px;height:"+height*2+"px;background-color:#FFFFFF'></div>")
	...
	if(this.$editable){
	    this.$workArea.on("click", { inthis: this }, function (e) {
	        ...
	        var now = new Date().format("ddhhmmss");
	        e.data.inthis.addNode(now + e.data.inthis.$max, { name: "node_" + e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, jdh: e.data.inthis.$max, zNodeId: 0, zNodeNname: "", zUserName: "", weight: null });
	        e.data.inthis.$max++;
	    });
	  //划线时用的绑定
...
//编辑节点页面
function nodePageEdit(NodeCode) {
    //  var strUrl = Zone.Web.WorkFlow.WorkFlowDesignView.getURL(Process_ID);
    //  LoadWindow("../" + strUrl.value + "?ACTIVITY_ID=" + Process_ID);
    var url = "NodePageEdit.aspx?NODE_CODE=" + NodeCode;
    window.open(url, '节点页面配置', 'height=400, width=600, top=200, left=200, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no') 
}
GooFlow.prototype = {
    useSVG: "",
    ...
    addNode: function (id, json) {
        ...
        if (json.type != "start" && json.type != "end") {
            json.width = json.width!=undefined ? json.width : 120;
            json.height = json.height!=undefined ? json.height : 45;
            if (json.top < 0) json.top = 0;
            if (json.left < 0) json.left = 0;
            var hack = 0;
            if (navigator.userAgent.indexOf("8.0") != -1) hack = 2;
            var tipShow = "not-show";
            if(json.weight != null)
            	tipShow = "show";
            var stateColor = " GooFlowStateNormal "; //不同状态颜色
            if (json.state) {
                switch (json.state) {
                    case nodeState.Normal:
                        stateColor = " GooFlowStateNormal ";//默认
                        break;
                    case nodeState.Undo:
                        stateColor = " GooFlowStateUndo ";//未做
                        break;
                    case nodeState.InProcess:
                        stateColor = " GooFlowStateInProgress ";//正在办理
                        break;
                    case nodeState.Reject:
                        stateColor = " GooFlowStateRegect ";//驳回审批
                        break;
                    case nodeState.ForceEnd:
                        stateColor = " GooFlowStateForceEnd ";//强制结束
                        break;
                    case nodeState.Pause:
                        stateColor = " GooFlowStatePause ";//暂停
                        break;
                    default:
                        stateColor = " GooFlowStateNormal ";
                        break;
                }
            }
			/*？？？？*/
			...
			param.icon = json.icon;

			if(param.weight == null)
			{
				this.$nodeDom[id] = $("<div class='GooFlow_item" + stateColor + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><div class='weight-tip "+tipShow+"'>权重："+json.weight+"</div><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr>结果值"+(param.icon*1).toFixed(4)+"</tr><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
			}else {this.$nodeDom[id] = $("<div class='GooFlow_item" + stateColor + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><div class='weight-tip "+tipShow+"'>权重："+(param.weight*1).toFixed(4)+"</div><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr>结果值"+ (param.icon*1).toFixed(4) +"*"+json.weight+"="+(json.weight*json.icon).toFixed(3)+"</tr><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");}
/*
            this.$nodeDom[id] = $("<div class='GooFlow_item" + stateColor + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><div class='weight-tip "+tipShow+"'>权重："+param.weight.toFixed(4)+"</div><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr>结果值"+ (param.icon*1).toFixed(4) +"*"+json.weight+"="+(json.weight*json.icon).toFixed(3)+"</tr><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
*/
            if (json.type.indexOf(" mix") > -1) this.$nodeDom[id].addClass("item_mix");
        }
        else if (json.type == "start") {
            this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><div class='weight-tip "+tipShow+"'>权重："+json.weight+"</div><table cellspacing='0'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>开始</div></div>");
        } else if (json.type == "end") {
            json.width = 24; json.height = 24;
            this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><div class='weight-tip "+tipShow+"'>权重："+json.weight+"</div><table cellspacing='0'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>结束</div></div>");
        }
    ...
    initWorkForNode: function () {
        ...
        /*this.$workArea.delegate(".GooFlow_item", "mouseenter", { inthis: this }, function (e) {
            var Next = GetActivityInfo(this.id).split(";");
            var menuList = [
	           { menuName: "节点名称： " + Next[1] },
	           { menuName: "经办人： " + Next[2] }
            ];

           var menuCount = menuList.length;
           if (!$("#myMenu")[0]) {
                var divMenuList = "<div id=\"myMenu\" class=\"div_RightMenu\"><div><ul class='ico' id='menuul'>";
                for (var i = 0; i < menuCount; i++) {
                    divMenuList += "<li style=\"list-style-type:none;\" >" + menuList[i].menuName + "</li>";
                }
                ...
                $("#menuul").remove();
                var mlnr = "<ul class='ico' id='menuul'>";
                for (var i = 0; i < menuCount; i++) {
                    mlnr += "<li  style=\"list-style-type:none;\" >" + menuList[i].menuName + "</li>";
                }
                mlnr += "</ul>";
                $("#myMenu").append(mlnr);

            }
            $("body").append(divMenuList).find("#myMenu").hide().find("li")
		            .bind("mouseover", function () { $(this).addClass("RM_mouseover"); })
		            .bind("mouseout", function () { $(this).removeClass("RM_mouseover"); });
            $(document).click(function () { $("#myMenu").hide(); });
            my = e.clientY;
            mx = e.clientX;
            $("#myMenu").hide().css({ top: my, left: mx }).show();
        });
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
            ...
            var now = new Date().format("yyMMddhhmmss");
            if (lineStart) This.addLine("line_" + now, { from: lineStart.id, to: this.id, name: "", zLineId: 0, zLineName: ""});
            This.$max++;
        });
        //绑定双击编辑事件
        ...
        this.$workArea.delegate(".ico + td", "dblclick", { inthis: this }, function (e) {
            ...
            This.$textArea.val(oldTxt).css({ display: "block", width: $(this).width() + 24, height: $(this).height(),
                left: t.left + 8 + This.$nodeData[id].left - This.$workArea[0].parentNode.scrollLeft,
        ...
        this.$workArea.delegate(".GooFlow_item", "contextmenu", { inthis: this }, function (e) {
            var menuList = [
	            { menuName: "对比阵编辑", menuclass: "1", clickEvent: "contrast('" + this.id + "')" },
                { menuName: "一致性校验", menuclass: "1", clickEvent: "verification('" + this.id + "')" },
                { menuName: "指标配置", menuclass: "1", clickEvent: "normConfig('" + this.id + "')" },
				{ menuName: "取消", menuclass: "1", clickEvent: "" } 
		    ];
            ...
            if (!$("#myMenu")[0]) {
                var divMenuList = "<div id=\"myMenu\" class=\"div_RightMenu\"><div><ul class='ico' id='menuul'>";
                for (var i = 0; i < menuCount; i++) {
                	if(menuList[i].menuclass == "1"){
                		divMenuList += "<li style=\"list-style-type:none;\" onclick=\"" + menuList[i].clickEvent + "\" class='"+menuList[i].menuclass+"'>" + menuList[i].menuName + "</li>";
                	}else{
                		divMenuList += "<li style=\"list-style-type:none; display:none;\" onclick=\"" + menuList[i].clickEvent + "\" class='"+menuList[i].menuclass+"'>" + menuList[i].menuName + "</li>";
                	}
                }
                divMenuList += "</ul></div></div>";
                ...
                var mlnr = "<ul class='ico' id='menuul'>";
                for (var i = 0; i < menuCount; i++) {
                    mlnr += "<li  style=\"list-style-type:none;\" onclick=\"" + menuList[i].clickEvent + "\">" + menuList[i].menuName + "</li>";
                }
                mlnr += "</ul>";
                $("#myMenu").append(mlnr);
            ...
            if (jq.prop("tagName") == "DIV") {
                ...
                if (GooFlow.prototype.useSVG != "") {
                    if (!this.$lineData[this.$focus].marked) {
                        jq[0].childNodes[1].setAttribute("stroke", "#a7a7a7");
                        jq[0].childNodes[1].setAttribute("marker-end", "url(#arrow1)");
                    }
                }
                else {
                    if (!this.$lineData[this.$focus].marked) jq[0].strokeColor = "#a7a7a7";
                }
    ...
    moveNode: function (id, left, top) {
        ...
        if (this.onItemMove != null && !this.onItemMove(id, this.$nodeData[id].zNodeId, "node", left, top)) return;
        if (this.$undoStack) {
    ...
    setName: function (id, name, type) {
        ...
        if (type == "node") {//如果是结点
            ...
            if (this.onItemRename != null && !this.onItemRename(id, this.$nodeData[id].zNodeId, name, "node")) return;
            ...
            if (this.onItemRename != null && !this.onItemRename(id, this.$lineData[id].zLineId, name, "line")) return;
            ...
            if (this.onItemRename != null && !this.onItemRename(id, this.$lineData[id].zAreaId, name, "area")) return;
    ...
    delNode: function (id) {
        ...
        if (this.onItemDel != null && !this.onItemDel(id, this.$nodeData[id].zNodeId, "node")) return;
        ...
        for (var k in this.$lineData) {
            if (this.$lineData[k].from == id || this.$lineData[k].to == id) {
                ...
                var item = {};
                item["line_id"] = this.$lineData[k].lineid;
                arrlines.push(item);
    ...
    drawLine: function (id, sp, ep, mark, dash) {
        ...
        if (GooFlow.prototype.useSVG != "") {
            ...
            if (mark) {
                ...
                path.setAttribute("stroke", "#a7a7a7");
    ...
    calcStartEnd: function (n1, n2) {
        var X_1, Y_1, X_2, Y_2;
        //X判断：
        var x11 = n1.left + n1.width/2, x12 = n1.left + n1.width/2, x21 = n2.left + n2.width*0.5, x22 = n2.left + n2.width*0.5;
        //结点2在结点1左边
        if (x11 >= x22) {
            X_1 = x11; X_2 = x22;
        }
        ...
        //Y判断：
        var y11 = n1.top + n1.height/2, y12 = n1.top + n1.height/2, y21 = n2.top + n2.height/2, y22 = n2.top + n2.height/2;
        //结点2在结点1上边
        if (y11 >= y22) {
            Y_1 = y11; Y_2 = y22;
        }
    ...
    addLine: function (id, json) {
        ...
        this.$lineData[id].lineid = json.lineid;
        this.$lineData[id].zLineId = json.zLineId;
    ...
    setLineType: function (id, newType) {
        ...
        //如果是变回直线
            ...
            if (this.onLineMove != null && !this.onLineMove(this.$lineData[id].zLineId, this.$lineData[id].type, 0)) return false;
        }
        if (this.$focus == id) {
            this.focusItem(id);
        }
    ...
    setLineM: function (id, M, noStack) {
        if (!this.$lineData[id] || M < 0 || !this.$lineData[id].type) return false;
        if (this.onLineMove != null && !this.onLineMove(id, this.$lineData[id].zLineId, this.$lineData[id].type, M)) return false;
    ...
    delLine: function (id) {
        if (!this.$lineData[id]) return;
        if (this.onItemDel != null && !this.onItemDel(id, this.$lineData[id].zLineId, "line")) return;
        if (this.$undoStack) {
