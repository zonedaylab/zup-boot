...
//	}
	...
	//接收url参数 ip为设备的ip port：端口号  user：用户名  psw：密码  channel：通道1~9  
	//$("#loginip").val(GetQueryString("ip"));
	//$("#port").val(GetQueryString("port"));
	//$("#username").val(GetQueryString("user"));
	//$("#password").val(GetQueryString("psw"));
	//$("#channelOrder").val(GetQueryString("channel")); 
    //var channel = GetQueryString("bujiancanshu");
	...
	$.ajax( {
		type : "post",
		async : false, //同步执行  
		url : "graph?videoparams",
       data: "bujianleixing="+GetQueryString("bujianleixing")+"&bujian="+GetQueryString("bujian")+"&changzhan="+GetQueryString("changzhan"),
		dataType : "json", //返回数据形式为json  
		success : function(result) {
			if (result) { 
				Params = result.attributes.deviceName[0].shuoMing;
				//Params = "192.168.10.250;80;admin;admin12345;1";
			} 
		},
	...
	strs=Params.split(";"); //字符分割 
	
	$("#loginip").val(strs[0]);
	$("#port").val(strs[1]);
	$("#username").val(strs[2]);
	$("#password").val(strs[3]);
	$("#channelOrder").val(GetQueryString("bujiancanshu")); 
...
function clickStartRealPlay() {
	...
	var iRet = WebVideoCtrl.I_StartRealPlay("192.168.10.250", {
		iWindIndex:0,
		iStreamType: 1,
		iChannelID: iChannelID,
		bZeroChannel: bZeroChannel
