...
//初始化区域图的对象
	...
	if(this.$editable){
	    this.$workArea.on("click", { inthis: this }, function (e) {
	        ...
	        e.data.inthis.addNode(now + e.data.inthis.$max, { name: "node_" + e.data.inthis.$max, left: X, top: Y, type: e.data.inthis.$nowType, jdh: e.data.inthis.$max, activityid: 0, activityname: "", username: "" });
	        e.data.inthis.$max++;
	    });
	  //划线时用的绑定
...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {
        ...
        if (json.type != "start" && json.type != "end") {
            json.width = json.width!=undefined ? json.width : 114;
            json.height = json.height!=undefined ? json.height : 58;
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
            ...
            if (lineStart) This.addLine("line_" + now, { from: lineStart.id, to: this.id, name: "" });
            This.$max++;
        });
        //绑定双击编辑事件
        ...
        this.$workArea.delegate(".GooFlow_item", "contextmenu", { inthis: this }, function (e) {
            var menuList = [
	            { menuName: "查看该节点页面", menuclass: "1", clickEvent: "ActivityFromView('" + this.id + "')" },
	            { menuName: "步骤基本属性", menuclass: "1", clickEvent: "ActivityEdit('" + this.id + "')" },
	            { menuName: "可写字段", menuclass: "1", clickEvent: "ActivityPermissionSet('" + this.id + "')" },
	            { menuName: "流转规则设置", menuclass: "1", clickEvent: "ActivityRuleSet('" + this.id + "')" },
	            { menuName: "通知者设置", menuclass: "1", clickEvent: "ActivityMessageSet('" + this.id + "')" },
	            { menuName: "操作者设置", menuclass: "1", clickEvent: "ActivityOperatorSet('" + this.id + "')" },
            ...
            if (!$("#myMenu")[0]) {
                var divMenuList = "<div id=\"myMenu\" class=\"div_RightMenu\"><div><ul class='ico' id='menuul'>";
                for (var i = 0; i < menuCount; i++) {
                    divMenuList += "<li style=\"list-style-type:none;\" onclick=\"" + menuList[i].clickEvent + "\">" + menuList[i].menuName + "</li>";
                }
                divMenuList += "</ul></div></div>";
    ...
    delNode: function (id) {
        if (!this.$nodeData[id]) return;
        var activityID= this.$nodeData[id].nodeid;
        if (this.$undoStack) {
        ...
        var par={};
        par.nodeId = activityID;
        par.arrLines = arrlines;
        $.ajax({ 
            dataType: "json",
            async: false, 
            url: "projectRegeditEditController/delNodesAndLines", 
            data: par,
            type: "POST", 
            cache: false,  //禁用缓存 
    ...
    delLine: function (id) {
        ...
        $.ajax({
            ...
            dataType: "json",
            async: false,
            url: "NavigationSchemeDesign.aspx/DelLines",
            data: "{'LineCode':'" + id + "'}",
            success: function (data) {
                alert(data.d);
            }
