...
<body>
<div class="nav nav_left" style="width: 260px;">
        ...
        <div class="nav_menu_content">
            <form class="nav_menu" id='resourceSelection'>
                <div class="nav_menu_item">
                    ...
                    <span title="设置表单">选择表单</span>
                    <i class="fa fa-angle-right" style=""></i>
                </div>
                    <div class="nav_menu_subitem"></div>
                </div>
                <div class="nav_menu_item">
                    <div class="nav_menu_item_title" type="2" child="true">
                        <span title="可选报表">可选报表</span>
                        <i class="fa fa-angle-right" style=""></i>
                    </div>
                    <div class="nav_menu_subitem"></div>
                </div>
                <div class="nav_menu_item">
                    <div class="nav_menu_item_title" type="3" subtype="1" child="true">
                        <span title="行维度">行维度</span>
                        <i class="fa fa-angle-right" style=""></i>
                    </div>
                    <div class="nav_menu_subitem"></div>
                </div>
                <div class="nav_menu_item">
                    <div class="nav_menu_item_title" type="3" subtype="2" child="true">
                        <span title="列维度">列维度</span>
                        <i class="fa fa-angle-right" style=""></i>
                    </div>
                    <div class="nav_menu_subitem"></div>
                </div>
                <div class="nav_menu_item">
                    <div class="nav_menu_item_title" type="3" subtype="3" child="true">
                        <span title="指标树">指标树</span>
                        <i class="fa fa-angle-right" style=""></i>
                    </div>
                    <div class="nav_menu_subitem"></div>
                </div>
                <div class="nav_menu_item">
                    ...
                    <div class="nav_menu_subitem">
                        <div class="nav_menu_item">
                            <div class="nav_menu_item_title" type="51" child="true">
...
<div id="show" style="height: 100%; width: 80%; background-color: #ffffff !important; float: right;  overflow: hidden;">
    <div id="box" style="height: 98%; width: 98%; margin-left: 10px; text-align: center;"></div>
</div>
...
<script>
    ...
    //处理列表tab切换
    $(".nav_btn_title").click(function () {
        var href = $(this).attr("href");
        if (href != "") {
            $(this).addClass("active").siblings(".nav_btn_title").removeClass("active");
            $(href).addClass("on").siblings(".nav_tab").removeClass("on");
        }
    });
    ...
    var page_Id = 0, report_Id = 0, block_Type = 0;
    //获取子菜单菜单元素
    function getSubGisMenu(elem) {
        var type = $(elem).attr("type");
        var pageId = $("input[name='form']:checked").val();
        page_Id = pageId;
        var reportId = $(".report:checked").val();
        report_Id = reportId;
        //判断是否有子菜单
                            ...
                            for (var i = 0; i < res.data.length; i++) {
                                ...
                                if (type == 1) {
                                    ...
                                    dom = '<div class="nav_menu_item">' +
                                        '<div class="nav_menu_item_title" data-id="' + res.data[i].bi_Page_Id + '" page_Type="' + res.data[i].page_Type + '" child="false">' +
                                        '<i class="fa fa-angle-right" style="' + icon + '"></i>' +
                                        '<input name="form" type="' + redioOrCheck + '" value="' + res.data[i].bi_Page_Id + '" data-menuId="' + res.data[i].menu_Id + '" ' + defaultChecked + ' style="' + checked + 'margin-right: 10px;" >' +
                                    ...
                                    dom = '<div class="nav_menu_item">' +
                                        '<div class="nav_menu_item_title" page-id="'+ res.data[i].page_Id +'" data-id="' + res.data[i].report_Id + '" child="false">' +
                                        '<i class="fa fa-angle-right" style="' + icon + '"></i>' +
                                        '<input type="' + redioOrCheck + '" class="report" value="' + res.data[i].report_Id + '" ' + defaultChecked + ' style="' + checked + 'margin-right: 10px;" >' +
                                    ...
                                    var subtype = $(elem).attr("subtype");
                                    if(res.data[i].display == 1){
                                        defaultChecked = "checked=\"checked\"";
                                    }
                                    console.log(res.data[i].display)
                                    if(subtype == res.data[i].field_Location){ //行维
                                        dom = '<div class="nav_menu_item">' +
                                            '<div class="nav_menu_item_title" data-id="' + res.data[i].report_Field_Id + '" child="false">' +
                                            '<i class="fa fa-angle-right" style="' + icon + '"></i>' +
                                            ...
                                            '<span title="' + res.data[i].field_Name + '">' + res.data[i].field_Name + '</span>' +
                                            '</div>' +
                                            '<div class="nav_menu_subitem"></div>' +
                                            '</div>';
                                    }else if(subtype == res.data[i].field_Location){ //列维
                                        dom = '<div class="nav_menu_item">' +
                                            '<div class="nav_menu_item_title" data-id="' + res.data[i].report_Field_Id + '" child="false">' +
                                            '<i class="fa fa-angle-right" style="' + icon + '"></i>' +
                                            '<input type="' + redioOrCheck + '" class="report" value="' + res.data[i].field_Id + '" ' + defaultChecked + ' style="' + checked + 'margin-right: 10px;" display="'+res.data[i].display+'" onchange="displayChange(this)" >' +
                                            ...
                                            '<div class="nav_menu_subitem"></div>' +
                                            '</div>';
                                    }else if(subtype == res.data[i].field_Location){ //指标
                                        dom = '<div class="nav_menu_item">' +
                                            '<div class="nav_menu_item_title" data-id="' + res.data[i].report_Field_Id + '" child="false">' +
                                            '<i class="fa fa-angle-right" style="' + icon + '"></i>' +
                                            '<input type="' + redioOrCheck + '" class="report" value="' + res.data[i].field_Id + '" ' + defaultChecked + ' style="' + checked + 'margin-right: 10px;" display="'+res.data[i].display+'" onchange="displayChange(this)" >' +
    ...
    function getList(id, indexs) {
        var index = loading.start("#1c6bab");
        $("#box").html("<h3 id=\"title\" style='margin: 10px;'></h3>" +
                            ...
                            if(typeof(re.data[0].dimFieldHeader[x][y]) === "undefined")
                                ...
                                if(thCon.indexOf("-") > -1){
                                    if((thCon.split("-")[1]+"").length == 6){
                                        theadTh += "<th title='"+thCon.split("-")[0]+"'><a class='sla' href='rest/mgeids/mgeidsMineInfoListController?county="+thCon.split("-")[1]+"'>" + thCon.split("-")[0]+"</a></th>";
                                    }else{
                                ...
                                if(typeof(re.data[z].tableData[i][j]) === "undefined"){
                                    ...
                                    if(tdCon.indexOf("-") > -1){
                                        if((tdCon.split("-")[1]+"").length == 6){
                                            tr += "<td><a href='rest/mgeids/mgeidsMineInfoListController?county="+tdCon.split("-")[1]+"'>" + tdCon.split("-")[0]+"</a></td>";
                                        }else{
    ...
    function displayChange(obj){
        ...
        $.ajax({
            ...
            data: {
                "report_Field_Id": then.parent().attr("data-id"),
                "display": dis,
                "report_Id": then.val(),
            },
            dataType: "json",
