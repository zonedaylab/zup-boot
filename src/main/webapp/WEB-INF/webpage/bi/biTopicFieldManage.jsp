    ...
    <script type="text/javascript">
		...
		<script type="text/javascript">
			...
			function getDBField(value){
	    		...
	    		$.ajax({
                    ...
                    success: function (result) {
                    	...
                    	for(var i=0; i<result.data.length; i++){
                    		if(value == result.data[i].dim_Field_Name){
								$("#field_Name").append("<option selected='selected' value="+result.data[i].dim_Field_Name+">"+result.data[i].dim_Field_Name+"</option>");
							}else{
								$("#field_Name").append("<option value="+result.data[i].dim_Field_Name+">"+result.data[i].dim_Field_Name+"</option>");
							}
						}
                    },
                    error: function(){
			...
			$("#field_Name").on("change",  function(){
				...
				$("#field_Caption").val($("#field_Name option:selected").text());
			});
