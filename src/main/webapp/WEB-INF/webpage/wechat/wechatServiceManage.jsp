...
<%--										<select id="systemIdQuery" class="form-control inputs">--%>
<%--							            </select>--%>
<%--									</div>--%>
									<div class="col-md-2 col-xs-2" style=" position:absolute; right:30px; top:0px">
									</div>						
			...
			<div class="row">
				<div class="col-md-12" style="margin-left:20px;height:30px; margin-bottom:10px; margin-top:5px;">
					...
					<button class="btn btn-primary btn-sm" id="btnToken">
						紧急加载Token
					</button>
				</div>
			</div>
				...
				<div class="col-md-12 column">
					<form class="form-horizontal" role="form">
						...
						<div class="form-group">
           				...
           				<div class="form-group">
							 <strong class="col-xs-12 col-md-3 control-label" style="text-align: left" >有效性 ：</strong>
							<div class="col-xs-12 col-md-8"> 
    ...
    <jsp:include page="../include/mainFooter.jsp"></jsp:include>
   	...
   	<script type="text/javascript">
        ...
        var param = {};
	       ...
	       //获取微信绑定系统名称
	        $.ajax({
	            ...
	            success: function (result) {
	       			$("#systemIdQuery").html(""); 
	       			$("#systemIdQuery").html("<option value='0'>全部</option>");
	       			$("#systemIdFrom").html(""); 
	            	for(var i=0;i<result.data.length;i++){
	             		$("#systemIdQuery").append("<option value='"+result.data[i].company_Id+"'>"+result.data[i].company_Name+"</option>");
	             		$("#systemIdFrom").append("<option value='"+result.data[i].company_Id+"'>"+result.data[i].company_Name+"</option>");
	            	}
	            },
	            error:function(){	            	
	            }
	    ...
	    function loadTableData(){
        	...
        	var table = $("#postTable").dataTable({
	            ...
	            ajax: function (data, callback, settings) {
					...
					if($("#systemIdQuery").val() != 0){
						param.system_Id = $("#systemIdQuery").val();
					}else{
						...
						},
						{
							...
							click: function() {
								if(thisid == "btnAdd"){
									...
									par.system_Id = $("#systemIdFrom").val();
									addWS(par);
								}
								if(thisid == "btnEdit"){
									par.service_ID = $("#service_ID:checked").val();
		...
		function lookWS(par){
			...
			$.ajax({
	            ...
	            success: function (result) {
					...
					$("#systemIdFrom").val(result.data.system_Id)
	            },
	            error: function(){
		...
		$("#btnToken").on("click", function(){
			$("#btnToken").html("<i class='fa fa-spinner fa-pulse' id></i>正在紧急加载Token...");
			setTimeout(function(){
				$.ajax({
		            url: 'rest/wechat/wechatServiceManageController/getWechatServiceToken',
		            async: false,
		            type: 'POST',
		            dataType: 'json',
		            success: function (result) {
						if(result.data == "success"){
							parent.bootbox.alert("紧急加载Token成功", function(){
								$("#btnToken").html("紧急加载Token");
							});
						}else{
							parent.bootbox.alert("紧急加载Token失败", function(){
								$("#btnToken").html("紧急加载Token");
							});
						}
		            },
		            error: function(){
		            	parent.bootbox.alert("紧急加载Token异常", function(){
		            		$("#btnToken").html("紧急加载Token");
			            });
		            }
	   	        });
