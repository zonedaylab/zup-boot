	...
	<!-- 左侧树形图加载数据js -->
		...
		<script type="text/javascript">
			...
			function getEnergyMarketInfo(){
				$.ajax({
			 	       url: 'rest/rbac/SaleMarketConfigController/getMarketInfo',
			           data: parup,
			           type: 'POST',
			           dataType: 'json',
				       success: function (result) {
			            	...
			            	for(var i=0;i<result.data.length;i++){
			             		$("#pickListResult").append("<option value='"+result.data[i].regionCodes+"'>"+result.data[i].region_Name+"</option>");
			            	}
				    	   
					       	if(thisid=="btnEdit"){//点击编辑按钮
					       		  if(result.result == "success"){
					       			$("#tradeMarketEdit").val(result.data[0].market_Name);
					              	$("#createTimeEdit").val(result.data[0].createTimeStr);
			...
			//获取未配置的区域
			 ...
			 function getUnSignedRegion(){
				 $.ajax({
			            url: 'rest/rbac/SaleMarketConfigController/listEnergyRegion',
			            data: parsign,
			            type: 'POST',
			            dataType: 'json',
			            success: function (result) {
			...
			function editCommon(parup){	
				...
				$.ajax({
	        		 type: "POST",
	                 url: 'rest/rbac/SaleMarketConfigController/updateMarketInfo', //用于文件上传的服务器端请求地址
	                 data: parup,
			 ...
			 function addCommon(paradd){
					...
					$.ajax({
		        		 type: "POST",
		                 url: 'rest/rbac/SaleMarketConfigController/addMarketInfo', //用于文件上传的服务器端请求地址
		                 data: paradd,
		                 secureuri: false, //是否需要安全协议，一般设置为false
		                 dataType: 'json', //返回值类型 一般设置为json
		                 success: function (result){  //服务器成功响应处理函数
			    	...
			    	pardel.energyMarketIds=[];
				 	parent.bootbox.confirm("你确定要删除这<font color=red>" + $("#market_Id:checked").length + "</font>个交易市场吗？",function(result){
			     		if(result){
							...
							$.ajax({
					 	       url: 'rest/rbac/SaleMarketConfigController/deleteEnergyMarket',
					           data: pardel,
					           type: 'POST',
					           dataType: 'json',
						       success: function (result) {
		...
		function getSelectInfo(){	
	    	   $.ajax({
	               type: "POST",
	               url: "rest/rbac/SaleMarketConfigController/getAllMarket", 
	               cache: false,  //禁用缓存
	               dataType: "json",
	               async:false,
	               success: function (result) {
	    	   ...
	    	   $.ajax({
	               type: "POST",
	               url: "rest/rbac/SaleMarketConfigController/getAllRegion", 
	               cache: false,  //禁用缓存
	               dataType: "json",
	               async:false,
	               success: function (result) {
		...
		function loadTableData(){
        	...
        	var table = $("#tradeMarketConfigTable").dataTable({
	            ...
	            ajax: function (data, callback, settings) {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "rest/rbac/SaleMarketConfigController/getRegionMarketPage",
	                    cache: false,  //禁用缓存
	                    data: param,  //传入组装的参数
	                    dataType: "json",
	                    success: function (result) {
