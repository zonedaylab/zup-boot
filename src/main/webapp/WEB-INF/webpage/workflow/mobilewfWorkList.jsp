...
<body>
		<header class="mui-bar mui-bar-nav">
	        ...
	        <h1 class="mui-title">待办工作</h1>
<!--	        <a class="mui-icon mui-icon-search mui-pull-right" style="width:10%; margin-right:2%" id="btnSerach"></a>-->
    	</header>
	  	<!--弹出框放置位置  添加 删除  查看详情-->
		<div id="popover" class="mui-popover" style="width:180px;">
		 	<button class="btn btn-sm btn-primary" type="button"  id="btnDetail" style="margin-left:10%"><i class="icon-ok"></i>查看详情</button>
		 	<button class="btn btn-sm btn-primary" type="button"  id="btnHandle"><i class="icon-ok"></i>办理</button>
		</div>
                   ...
                   success: function (result) {
	                   ...
	                   if(result.data.length < 1){
	                    ...
	                    for(var i=0;i<result.data.length;i++){
	                   		...
	                   		$("#mainMenu").append('<li class="mui-table-view-cell" id='+result.data[i].WORK_ID+'><div class="mui-slider-right mui-disabled"><a class="mui-btn mui-btn-grey mui-icon  mui-icon-plus" style="font-size: 18px; padding: 5px;" onclick="javascript:btnHandle(this);">详情</a><a class="mui-btn mui-btn-yellow mui-icon mui-icon-compose" style="font-size: 18px; padding: 5px;" onclick="javascript:btnDetail(this);">办理</a></div><div class="mui-slider-handle"><div class="mui-table-cell mui-text-left" style="float: left;"><span class="mui-ellipsis" style="font-family: "黑体"; font-size:18px;">['+result.data[i].WORK_NAME+']-'+result.data[i].RECEIVER_NAME+'</span><h5><span style="width: 200px; color: #000000; font-size:14px;">'+result.data[i].WORK_ITEM_DATETIME+'</span></h5></div></div></li>');
	                   } 
	                   mui.init();
                   }
               });
